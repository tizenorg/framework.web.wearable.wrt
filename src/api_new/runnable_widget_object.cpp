        ...
        m_contextManagerFactoryMethod(ViewModule::makeContextManagerFactoryMethod())
{
    ...
    if (UID_ROOT == getuid()) {
        ...
        if (!ClientModule::SecuritySupport::setAppPrivilege(
...
void RunnableWidgetObject::TimeTick(long time)
{
    LogDebug("TimeTick called");
    // TODO: how to handle state guard?????
    m_view->TimeTick(time);
}

void RunnableWidgetObject::AmbientTick(long time)
{
    LogDebug("AmbientTick called");
    m_view->AmbientTick(time);
}

void RunnableWidgetObject::AmbientModeChanged(bool ambient_mode)
{
    LogDebug("AmbientModeChanged called");
    m_view->AmbientModeChanged(ambient_mode);
}
