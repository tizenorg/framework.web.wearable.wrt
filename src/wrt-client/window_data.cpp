...
void WindowData::initTheme(void)
{
    const Eina_List* list = elm_theme_list_get(NULL);
    if (!list) {
        LogWarning("elm_theme_list_get fail");
        return;
    }

    const char* theme = static_cast<const char*>(eina_list_data_get(list));
    if (!theme) {
        LogWarning("eina_list_data_get fail");
        return;
    }

    char* path = elm_theme_list_item_path_get(theme, NULL);
    if (!path) {
        LogWarning("elm_theme_list_item_path_get fail");
        return;
    }

    Eina_Bool isExist = edje_file_group_exists(path, "*");
    if (!isExist) {
        LogDebug("theme path: " << path);
    }
    if (path) {
        free(path);
    }
    ...
    case APP_CATEGORY_IDLE_CLOCK:
        {
            ...
            name << pid;
            // should I add return value of watch_app_get_elm_win() function?
            // there are no error handling code of caller function.
