    ...
    m_submodeSupport(new ClientModule::SubmodeSupport())
{
    ...
    m_tizenId = ClientModule::CommandLineParser::getTizenId(argc, argv);

    LogDebug("m_tizenId : " << m_tizenId);
...
void WrtClient::OnTimeTick(watch_time_h watch_time)
{
    time_t time;
    ...
    if (!ret) {
        LogDebug("time : " << time);
        m_widget->TimeTick(time);
    } else {
        LogDebug("Fail to get utc time. skip send tick event");
    }
}

void WrtClient::OnAmbientTick(watch_time_h watch_time)
{
    time_t time;
    int ret = watch_time_get_utc_timestamp(watch_time, &time);
    if (!ret) {
        LogDebug("time : " << time);
        m_widget->AmbientTick(time);
    } else {
        LogDebug("Fail to get utc time. skip send ambient tick event");
    }
}

void WrtClient::OnAmbientChanged(bool ambient_mode)
{
    m_widget->AmbientModeChanged(ambient_mode);
}
...
void WrtClient::launchStep()
{
    ...
    AppCategory type = APP_CATEGORY_NORMAL;
    if (m_isWatchApp) {
        type = APP_CATEGORY_IDLE_CLOCK;
    }
...
void WrtClient::shutdownStep()
{
    ...
    // to avoid call handler after destroy context, unregister callback
    vconf_ignore_key_changed(VCONFKEY_SYSMAN_LOW_MEMORY, vconf_lowmem_handler);
...
int WrtClient::languageChangedCallback(void *eventInfo, void *data)
{
...
bool WrtClient::watch_app_create(int width, int height, void *data)
{
    LogDebug("enter : width("<<width<<"), height("<<height<<")");
    return Application::app_create(data);
}

void WrtClient::app_time_tick(watch_time_h watch_time, void *data)
{
//do not fire tick event for normal clock for web app    
#if 0
    LogDebug("enter");
    WrtClient *This = static_cast<WrtClient *>(data);
    This->OnTimeTick(watch_time);
#endif
}

void WrtClient::app_ambient_tick(watch_time_h watch_time, void *data)
{
    LogDebug("enter");
    WrtClient *This = static_cast<WrtClient *>(data);
    This->OnAmbientTick(watch_time);
}

void WrtClient::app_ambient_changed(bool ambient_mode, void *data)
{
    LogDebug("enter");
    ...
    This->OnAmbientChanged(ambient_mode);
}
...
int WrtClient::Exec()
{
    ...
    if (!m_isWatchApp) {
        ...
        LogDebug("Starting watch application framework...");
        watch_app_lifecycle_callback_s callback;
        callback.create = watch_app_create;
        ...
        result = watch_app_main(m_argc, m_argv, &callback, this);
        LogDebug("Exited application framework");
    }
    return result;
}
         ...
         char *argv[])
{
    ...
    UNHANDLED_EXCEPTION_HANDLER_BEGIN
    {
        ...
        if (argc > 1 && argv[1] != NULL && !strcmp(argv[1], "-d"))
        {
            ...
            std::string backupEnv = getenv(HOME);
            if (backupEnv.empty()) {
            ...
            LogDebug("elm_init()");
            elm_init(argc, argv);
            setenv(HOME, backupEnv.c_str(), 1);

            LogDebug("WindowData()");
           s_preparedWindowData = new WindowData(APP_CATEGORY_NORMAL, static_cast<unsigned long>(getpid()));
           s_preparedWindowData->initTheme();
            ...
            if (pluginProcessPath.empty()) {
                LogError("Failed to get plugin process path");
                exit(-1);
            }
